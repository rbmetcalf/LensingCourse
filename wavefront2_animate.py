...
class Wavefront :
    ...
    def step(self) :
        self.x,self.v = step(self.x,self.v)
        return self.x,self.v
        
        
wavefront = Wavefront(x,v)

fig, ax = plt.subplots(figsize=(5,5))
def frame(i) :
#def frame(i,x,v,step) :
    ...
    ax.plot(genx(x),geny(x))
    ax.plot(genx(ray1),geny(ray1),linestyle='--')
    ax.plot(genx(ray2),geny(ray2),linestyle='--')
    ax.plot(genx(ray3),geny(ray3),linestyle='--')
    ax.plot(genx(ray4),geny(ray4),linestyle='--')

    ax.plot(x_lens.x,x_lens.y,'o')
    ax.set_xlim(-0.2,0.2)
    ax.set_ylim(1.15,1.6)
    
ani = FuncAnimation(fig,frame, frames=20, interval=200, repeat=True)
